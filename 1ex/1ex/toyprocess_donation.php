...
if (isset($_POST['submit'])) { // Changed from 'submit' to 'donationStatus'
    ...
    if (empty($toy)) {
        $errors[] = "Name of toy item is required.";
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO donations (campaignId, campaignTitle, name, email, location, toy, age, conditions, description, collectionMethod, username, status, donationStatus)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssssssss", $campaignId, $campaignTitle, $name, $email, $location, $toy, $age, $conditions, $description, $collection_method, $username, $status, $donationStatus);
        $stmt->execute();
        $stmt->close();
