...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2><center>CLOTHES <span style="color:orange;">CAMPAIGNS</center></h2>
            ...
            if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["delete"])) {
                // Get the ID of the donation to be deleted
                $deleteId = $_POST["delete"];

                // Perform the campaign deletion in the database
                $sql = "DELETE FROM campaigns WHERE id = $deleteId";
            
                if ($conn->query($sql) === TRUE) {
                    echo "<script>alert('Campaign deleted successfully.'); window.location.href = 'clothcamplist.php';</script>";
                } else {
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<form action='' method='post' onsubmit='return confirmDelete()' style='display: inline;'>";
                            echo "<input type='hidden' name='delete' value='$campaignId'>";
                            echo "<button type='submit' style='background: none; border: none; cursor: pointer;'><i class='fa-sharp fa-solid fa-trash' style='color: #f00000;'></i></button>";
                            echo "</form> | ";
                            echo "<a class='view-donations-link' href='clothdon-bycampaign.php?campaign_id=$campaignId&campaign_title=" . urlencode($row["campaignTitle"]) . "&beneficiaries=" . urlencode($row["beneficiaries"]) . "&beneficiaries_location=" . urlencode($row["beneficiariesLocation"]) . "&beneficiaries_phone=" . urlencode($row["beneficiariesPhone"]) . "&campaign_end_date=" . urlencode($row["campaignEndDate"]) . "&donation_status=" . urlencode($row["donationStatus"]) . "&updated_donation_status=true'>View Donations</a>";
                            echo "</div>";
                            echo "</td>";
                            echo "<td>";
    ...
    <script>
        function editCampaign(campaignId) {
            ...
            window.location.href = "editc_campaign.php?id=" + campaignId;
        }
        ...
        function addCampaign() {
            ...
            window.location.href = "addclothcamp.php";
        }
