...
if (isset($_POST['submit'])) { // Changed from 'submit' to 'donationStatus'
    ...
    if (empty($food)) {
        $errors[] = "Name of food item is required.";
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO donations (campaignId, campaignTitle, name, email, location, food, amount, description, collectionMethod, username, status, donationStatus)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssssss", $campaignId, $campaignTitle, $name, $email, $location, $food, $amount, $description, $collection_method, $username, $status, $donationStatus);
        $stmt->execute();
        $stmt->close();
