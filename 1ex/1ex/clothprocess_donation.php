...
if (isset($_POST['submit'])) { // Changed from 'submit' to 'donationStatus'
    ...
    $clothes= sanitizeInput($_POST['clothes']);
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO donations (campaignId, campaignTitle, name, email, location, clothes, amount, description, collectionMethod, username, status, donationStatus)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssssss", $campaignId, $campaignTitle, $name, $email, $location, $clothes, $amount, $description, $collection_method, $username, $status, $donationStatus);
        $stmt->execute();
        $stmt->close();
